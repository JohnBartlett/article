...
name: retrospective
description: >
  End-of-session (or any-time) lessons-learned pass — memorialize what went wrong and
  what went right in this session, then draft updates to CLAUDE.md, skills, and memory
  files for the user's review before applying anything. Use when the user asks to run a
  retrospective, capture lessons learned, "perfect the rules," or invokes /retrospective.
user-invocable: true
---
...
**Memory files:**
```bash
cat /Users/john/.claude/projects/-Users-john-article/memory/MEMORY.md
```
...
2. **Skill files** — edit `.claude/commands/` (legacy flat-file skills) or `.claude/skills/<name>/SKILL.md`
   (current convention) — only the specific sections that changed
3. **Memory files** — write new files or update existing ones in
   `/Users/john/.claude/projects/-Users-john-article/memory/`; update `MEMORY.md` index

Commit after all changes are applied:

```bash
git add CLAUDE.md .claude/commands/ .claude/skills/
