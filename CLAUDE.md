...
21. **Astrochart (daily-star) must also be copied each week** — Like the DateBook, the Astrochart folder (`daily-star-MONTH/`) must be copied from the previous edition: `cp -r editions/PREV-DATE/daily-star-MONTH editions/NEW-DATE/daily-star-MONTH`. If it is missing, the Astrochart link is a 404. Do this at edition setup, same time as DateBook copy. After copying: (a) delete all `<section>` blocks and `<option>` entries with dates before the current edition date, and (b) verify coverage extends through the end of the current month. If data is missing, add to the To Do list: "Email Victoria (`vconst@aol.com`) for [Month] forecast — current data ends [date]." A 1 KB .docx from Victoria is empty/corrupt — always check file size before trusting it. **Since Sept 2026, the page's Prev/Next/dropdown date navigation fires a `gtag('event', 'astro_day_change', {day, method})` GA4 event (added to enable per-visit engagement analysis — how many days a reader clicks through, not just that they loaded the page).** This is inline JS copied forward with the rest of the file, so the copy-forward step preserves it automatically — but if a future astrochart page is ever rebuilt from scratch instead of copied, re-add this event handler (see `editions/2026-09-06/daily-star-september/index.html` for the reference implementation) rather than reverting to the untracked version.
...
61. **A contributor's own submitted title can be silently overridden by Judy's internal lineup placeholder name, with no audit catching it.** Four articles in the Sept 20 edition (`baseball-ulysses`, `basak-notz`, `newberry-chicago-machine`, `block-museum`) were published under Judy's working-list name for that slot instead of the actual title the writer/Judy used in their submitted article. Example: David Sweet's manuscript was headlined "An Author Finds Connection Between Wrigley Field and the Ancient Greeks," but the site published it as "Baseball and Ulysses" (Judy's lineup name). Two of the four were already flagged as open, unconfirmed questions in STATUS.md at build time and simply never resolved; the other two were never even flagged. None of `verify_edition.py`/`edition_checks.py`/`content_audit.py` catch this, because none of them compare the built title against the source email — they only validate internal site structure. Caught only because David complained directly to Judy. Before marking any article Ready, compare its H1/title against the source email's own subject line or in-body title, not just Judy's lineup list — treat a mismatch as an open question requiring confirmation, the same as photo placement ambiguity.

62. **HTML email plain-text extraction silently drops italics/bold, the same failure family as PDF extraction (mistake #43), just triggered by a different source format.** `baseball-ulysses` lost 8 italicized terms (book/film titles, a Latin word), `basak-notz` had one phrase built as italic when the source used bold, and `block-museum`/`newberry-chicago-machine` each lost one italicized term — all because the original builds read the email's plain-text body instead of its HTML body, which silently strips every `<i>`/`<b>`/`<em>`/`<strong>` tag with zero visible sign anything is missing (the plain text still reads as normal, grammatically correct English). When building an article from a Gmail message, always fetch `FULL_CONTENT` (or otherwise inspect the HTML body) and check for `<i>`/`<b>`/`<em>`/`<strong>` tags or inline `font-style:italic`/`font-weight:700` styles before treating the plain-text extraction as complete — this generalizes mistake #43's PDF guidance to HTML email sources too.

63. **A stale `thumb-placeholder.jpg` in `about.html`'s Articles-popup mini-cards is the same "never revisited after real assets arrived" bug as mistake #60's homepage teaser, in a second location.** Three of the Sept 20 edition's mini-card images (`baseball-ulysses`, `hawthorne-works`, `bob-glaze`) still pointed at the generic placeholder image in `about.html` even though real `navthumb.jpg`/cover photos existed for all three — found incidentally, not by any audit. The same pattern exists on several older editions (Sept 6, Sept 13 — at least 10 more instances) but was left alone as out of scope. `content_audit.py`'s stale-teaser check (mistake #60) should be extended to also flag any `about.html` mini-card `<img>` still pointing at `thumb-placeholder.jpg`/`card-placeholder.jpg` for an article that's Ready.

64. **A single-file `git checkout <branch> -- <file>` hotfix from `dev` onto `master` always brings the GA4-disabled form, and it's easy to forget to fix.** Every hotfix pushed straight to `master` during the Sept 20 session (DateBook events, title fixes, formatting fixes) required manually re-enabling GA4 after the file-level checkout, because `dev`'s copy of every HTML file carries the `<!-- GA4-disabled ... -->` wrapper. This was caught every time only by deliberately checking — it would be trivial to skip once and ship GA4-disabled to production undetected until the next `/stage` cycle. After any `git checkout dev -- <file>` onto master, always grep the pulled file(s) for `GA4-disabled` before committing, and re-enable it as a mandatory step, not an optional afterthought.
