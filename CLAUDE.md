...
24. **Never put internal links in the public hamburger menu** — The Editors' Page, Stats, Reader Comments, and Future Articles links are internal tools. They belong only in the `<!-- dev2-only -->` internal-nav bar, never in the public `<div class="hamburger-menu">`. The hamburger must contain only: About, Subscribe, Advertise.
25. **Nav pattern: About/Subscribe/Advertise go in hamburger ONLY, not nav-inner** — The `<div class="nav-inner">` must contain: Home, DateBook, Astrochart, hamburger button. About/Subscribe/Advertise must appear only inside `<div class="hamburger-menu">`. Putting them in both causes a doubled menu visible to all readers. `verify_edition.py` now checks for this.
