...
1. **Two missing `about.html` bio anchors** — `michael-traynor` and `ronald-clewer` have no bio cards at all, so their byline/About-the-Author links currently 404. Need at minimum a name + one-line role for each before these two articles can be considered fully clean, though they're otherwise Ready and publishable.
