...
| cheryl-menton-garden | Jardin Botanique Val Rameh | Cheryl Anderson | ✅ Full text built (Aug 15) | ⚠️ 12/14 placed (Aug 15) | Full article received via Annie (thread `1a005710fbbe464f`), approved by Judy. 12 of 14 Drive-linked photos downloaded successfully. **2 still missing:** Photo 7 (Olive tree) has a valid Drive link but the download call failed 5 times in a row while every other file succeeded (including larger ones) — looks like a transient issue with that specific object, worth one more retry later. Photo 11 (Pond 2) has no Drive link in the email at all — likely a contributor omission, not fabricated or guessed. `verify_edition.py` correctly flags the missing Photo 7 as a broken image reference. Contributor title is "Jardin Botanique Val Rameh" (email subject uses "Val Rameh" but body text uses "Val Rahmeh" with extra h — not corrected, flagged here). |
