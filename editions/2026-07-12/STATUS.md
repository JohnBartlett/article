...
| 2 | biba-roesch | Biba's Favorite Things &ndash; Steven Zick | Biba Roesch | Emma | ✅ | ✅ 4 photos | Emma sent a Mia Cohen piece first then retracted it ("saved for a future week"), then sent the correct Steven Zick piece (msg `19f4f1ab6d329848`) matching the existing cover photo (Biba with Steve Zick); built Jul 11, full text + 3 inline photos (Christie's, Giotto & Toots). Judy flagged the Giotto/Toots photo (`photo 3.png`) as sideways (msg `19f52bc434b128c3`), then the user confirmed it was still sideways specifically on iPhone/Safari, "bottom of the photo on the left" — the real bug: raw pixels were always upright (confirmed by direct visual inspection multiple times), but the file carried a spurious EXIF orientation=6 tag that Safari/iOS honors (rotating an already-correct image) while Chrome ignores (which is why an earlier check in Chrome looked fine and wasn't actually verifying the real-world bug). Fix: strip the EXIF orientation tag without touching pixel data (commit 8b1dcc7a). A parallel session, working from an interim report that the live site was "still sideways" (true at the time — the EXIF-strip fix hadn't reached production yet), concluded pixels needed physical 90° rotation and applied that instead (commit 4b60a8c) — **that fix is wrong and produces a sideways image in Chrome and every other EXIF-ignorant renderer**, confirmed by direct visual inspection. Resolved the conflict in favor of the EXIF-strip-only fix when merging the two sessions' work. |
