...
_Updated: 2026-09-19 (Annie's Newberry/Dominic Pacyga text arrived — full text placed, all 8 nav-linked articles now have text; 7 photos pending staging)_

**✅ Newberry Library / Dominic Pacyga — full text placed, article Ready.** Annie sent the complete article (`1a0bb9b780c3dc19`, 9:38 PM UTC) — a profile of historian Dominic Pacyga's *Clout City: The Rise and Fall of the Chicago Political Machine* winning the 2026 Pattis Family Foundation Chicago Book Award. **The email itself states "Author: Judy Carmack Bross"** — this is a Judy-authored profile piece (same pattern as `basak-notz`), not an article written by Pacyga himself, so the byline and About the Author link were set to Judy's existing `about.html#judy-carmack-bross` bio, not a new Dominic Pacyga bio card. **This resolves the "Dominic Pacyga needs a bio" blocker as moot** — no new bio is needed since he isn't the byline author. The email gives explicit "Photo One" through "Photo Seven" placement labels with verbatim captions for all 7 photos (`1-Dominic Headshot.jpeg` through `7-Three people.jpeg`) — an unambiguous map, recorded as HTML comments at each anchor in `index.html` pending staging (arrived after the hourly `_attachment-staging/` fetch window, not yet available to copy in). The existing `DominicHeadshot.jpeg` already in the folder (from an earlier, separate email) remains the homepage card image; unconfirmed whether it's the same photo as this email's "1-Dominic Headshot.jpeg" (intended as the hero) — not assumed either way. Several verbatim-but-flagged typos in the source per house rule #18, left uncorrected: "Bathhouse John Couglin" (Coughlin), "yamulke" (yarmulke), "Richand M. Daley" (Richard), "Chicago soon because" (became), "1871changed" (missing space), a missing period after "complexities", and an incomplete sentence ("Wald, who is professor of American Studies..."). `verify_edition.py` confirms `newberry-chicago-machine` **READY** (content + 1 photo on disk; 7 more pending staging).
...
above; all 25 photos placed. Dominic Pacyga still needs an about.html bio.
...
- ~~**The Newberry Library: A Book on the Chicago Machine** (Judy Carmack Bross, profiling Dominic Pacyga)~~ — done, article **Ready**. Full text received `1a0bb9b780c3dc19`, 9:38 PM UTC. See top summary — byline corrected to Judy per the email's own "Author:" line; 7 photos mapped from explicit "Photo N" labels, pending `_attachment-staging/` catch-up.
...
| newberry-chicago-machine | The Newberry Library: A Book on the Chicago Machine | Judy Carmack Bross (profiling Dominic Pacyga) | **Ready** | 1/8 in folder (`DominicHeadshot.jpeg`, homepage card); 7 more mapped from explicit "Photo N" labels, pending `_attachment-staging/` | Byline corrected to Judy per the source email's own "Author:" line |
