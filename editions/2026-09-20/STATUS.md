...
- ~~**Baseball and Ulysses** (David A. F. Sweet)~~ — article text now received and built verbatim (`1a0ab428d33b38b8`). Three captioned photos (`Baseball 1 2.jpeg`, `Baseball 2.jpeg`, `Baseball 3.jpeg`) attached with captions given in order — not yet in `_attachment-staging/` as of this pass (email arrived after the most recent hourly fetch), and placement is blocked regardless: see Blockers below. Article is **Text Only**.
...
| baseball-ulysses | Baseball and Ulysses | David A. F. Sweet | **Text Only** | Pending — 3 captioned photos not staged yet, main-photo question open | Hero photo choice unresolved |
