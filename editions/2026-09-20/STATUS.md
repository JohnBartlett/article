...
_Updated: 2026-09-20 (edition published to production — commit `be806245`, John's notification sent `1a0bc78e5ddad5ff`; DateBook events, David Sweet's headline/italics, and the other title/formatting fixes have all since been hotfixed to master by a separate session — confirmed live per John's explanation email `1a0bffbc1f3d1d0c`)_

**✅ Edition published to production.** `origin/master` commit `be806245c681521e514770ee2b8f82deb429cfa4` ("Publish September 20 edition to production," Sept 19 9:31 PM ET) confirms the stage→publish pass ran and GA4 is re-enabled on master. John's own live-announcement email (`1a0bc78e5ddad5ff`, Sept 20 1:40 AM UTC, to Judy/Sig/Emma/Annie/Ana) confirms the same. All 7 nav-linked articles are live.

**✅ DateBook — 5 new events added to dev2, now confirmed hotfixed to production.** Annie's reply (`1a0bc5483e891470`, Sept 20 12:00 AM UTC / "Re: datebook") answered John's "yes, send additions anytime" (item #212) with a 20-event list covering Sept 24 – Dec 12. 15 of the 20 were already on the page (carried forward from last week's datebook before this email arrived); 5 are genuinely new and were added to `editions/2026-09-20/datebook/index.html` on dev2 at their correct chronological anchors: Oct 15 St Leonard's Ministries ("A Night of Fellowship"), Oct 22 Costume Council of the Chicago History Museum ("Threads of a Nation"), Oct 24 The Auditorium/Chicago Philharmonic ("Edward Scissorhands in Concert"), Nov 3 Driehaus Museum ("CSO Chamber Music: Carving Out History"), Nov 6 Community House Winnetka ("The Winnetka Edit"). A separate (non-cloud) session has since pushed the same DateBook additions to master directly (`origin/master` commit `3653e1e`, "Hotfix: DateBook updates from Annie (5 new events, dedup fix)") — verified: master and dev2 both show 39 `event-entry` blocks in `datebook/index.html`. No further hotfix needed. Manual audit also found a duplicate "Red Line Jazz Project" entry (Sept 10 and Oct 1, same org/venue/near-identical description) and a truncated Harris Theater ticket URL (missing the final "l" in "festival") — flagged to Annie by email (`1a0bee9c6f5f5dff`), not fixed pending her answer.

**✅ Baseball and Ulysses — headline changed, book/film titles italicized, now confirmed hotfixed to production.** David Sweet emailed Judy directly (`1a0bf869c19791d6`, fwd from `dafsweet@aol.com`, Sept 20 10:04 AM CDT) asking for two fixes to his own published article: (1) the headline should read "An Author Finds Connection Between Wrigley Field and the Ancient Greeks" instead of "Baseball and Ulysses" — his words: "For some reason, my headline was changed" — and (2) book/film titles ("The Ancient Wisdom of Baseball," "The Odyssey," and shorthand "Ancient Wisdom" references) should be italicized — his words: "I italicize the names of books, and those keep getting removed." Both applied on dev2: `<title>`/`<h1>` updated in the article, propagated to the homepage card (`index.html`) and the About page's Articles popup (`about.html`) — slug/URL unchanged. All 6 book/film title mentions wrapped in `<em>`, including one inside a direct quote (title italicization is typographic convention, not a verbatim-text change). A separate (non-cloud) session pushed this same headline fix straight to master (`origin/master` commit `4e900e3`, "Hotfix: restore David Sweet's own headline on Baseball and Ulysses"), plus 3 more title restorations and formatting/italics fixes across the edition (`9ba6ae3`, `394cca2`) — confirmed live: `origin/master`'s `baseball-ulysses/index.html` `<title>`/`<h1>` now reads "An Author Finds Connection Between Wrigley Field and the Ancient Greeks." John's own explanation email to Judy (`1a0bffbc1f3d1d0c`, Sept 20 6:02 PM UTC, "September 20th edition: what happened and how it was fixed") confirms all 7 articles are checked and correct in production — no further hotfix needed. **Incidental finding while fixing `about.html`:** 3 of this edition's mini-card images (`baseball-ulysses`, `hawthorne-works`, `bob-glaze`) were still pointing at `thumb-placeholder.jpg` even though real `navthumb.jpg`/cover images exist — fixed for these 3; the same pattern exists across several older editions (Sept 6, Sept 13) but was left alone as out of scope for this pass.
...
- **Photo credit corrected: "Erika Dufou" → "Erika Dufour"** in `basak-notz/index.html` (line 457, photo credit line), per Basak Notz's own request relayed by Judy (`1a0c384d2bdb8842`, Sept 21). Applied on dev2 only, per this session's scope (never push to master). Since the article is already live in production, this **still needs the same dev/master hotfix** as the other Sept 20 post-publish fixes (items #216/#219/#221 pattern) — flagged for John.
- **Heritage Auctions homepage ad swapped: June → October creative.** Judy forwarded (`1a0d03cdacb6b128`, Sept 23 9:40 PM UTC) Heritage's Heidi Calkins with the new `Classic Chicago October 2026.png` ad and instruction "Please link it to HA.com/5587," saying "we can start it this Sunday, September 27." Applied on dev2 only (item #237): `index.html` hero-ad now points to `ads/Classic Chicago October 2026.png` linking to `https://www.ha.com/5587`, replacing the June comics-auction creative/link. Since the homepage is already live in production with the old ad, and Judy wants the new one live by Sept 27 (this Sunday), this **needs a dev/master hotfix** alongside the Erika Dufour fix above — flagged for John.
