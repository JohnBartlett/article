...
_Updated: 2026-09-20 (edition published to production — commit `be806245`, John's notification sent `1a0bc78e5ddad5ff`; 5 new DateBook events added to dev2 post-publish, need a hotfix to master)_
...
**🔁 DateBook — 5 new events added to dev2, not yet on production.** Annie's reply (`1a0bc5483e891470`, Sept 20 12:00 AM UTC / "Re: datebook") answered John's "yes, send additions anytime" (item #212) with a 20-event list covering Sept 24 – Dec 12. 15 of the 20 were already on the page (carried forward from last week's datebook before this email arrived); 5 are genuinely new and were added to `editions/2026-09-20/datebook/index.html` on dev2 at their correct chronological anchors: Oct 15 St Leonard's Ministries ("A Night of Fellowship"), Oct 22 Costume Council of the Chicago History Museum ("Threads of a Nation"), Oct 24 The Auditorium/Chicago Philharmonic ("Edward Scissorhands in Concert"), Nov 3 Driehaus Museum ("CSO Chamber Music: Carving Out History"), Nov 6 Community House Winnetka ("The Winnetka Edit"). Since the edition is already live in production (see above), per John's own precedent answer to Annie (item #212, "a post-publish hotfix works too"), these 5 events still need to be hotfixed to `dev`/`master` in a follow-up session — not done here per this skill's dev2-only scope. Manual audit also found a duplicate "Red Line Jazz Project" entry (Sept 10 and Oct 1, same org/venue/near-identical description) and a truncated Harris Theater ticket URL (missing the final "l" in "festival") — flagged to Annie by email (`1a0bee9c6f5f5dff`), not fixed pending her answer.

**🔁 Baseball and Ulysses — headline changed, book/film titles italicized, still needs a production hotfix.** David Sweet emailed Judy directly (`1a0bf869c19791d6`, fwd from `dafsweet@aol.com`, Sept 20 10:04 AM CDT) asking for two fixes to his own published article: (1) the headline should read "An Author Finds Connection Between Wrigley Field and the Ancient Greeks" instead of "Baseball and Ulysses" — his words: "For some reason, my headline was changed" — and (2) book/film titles ("The Ancient Wisdom of Baseball," "The Odyssey," and shorthand "Ancient Wisdom" references) should be italicized — his words: "I italicize the names of books, and those keep getting removed." Both applied on dev2: `<title>`/`<h1>` updated in the article, propagated to the homepage card (`index.html`) and the About page's Articles popup (`about.html`) — slug/URL unchanged. All 6 book/film title mentions wrapped in `<em>`, including one inside a direct quote (title italicization is typographic convention, not a verbatim-text change). Since the edition is already live, this needs the same dev/master hotfix as the DateBook additions above. **Incidental finding while fixing `about.html`:** 3 of this edition's mini-card images (`baseball-ulysses`, `hawthorne-works`, `bob-glaze`) were still pointing at `thumb-placeholder.jpg` even though real `navthumb.jpg`/cover images exist — fixed for these 3; the same pattern exists across several older editions (Sept 6, Sept 13) but was left alone as out of scope for this pass.
