...
- **Chef** (Sigalit Zetouni) — Blast text received Sept 8 (msg `1a08369f62e68b76`): one paragraph about NYB hamantaschen/Jacob Shaw. Full article still not received; placeholder still in place — nothing more to build until it arrives. Her one photo (`IMG_7976.JPEG`) is on disk and now used for the article's own nav thumbnail.
...
- **Nostalgia and the Village** (Laurel Baer) — via Annie. Not received. (Note: Judy's final list spells it "Laurel Baer", not "Baar" — update if confirmed.) Annie's Sept 12 5:54 AM ET promise of drafts "by noon" has not yet been checked against a later email pass — see next `/check-emails` run.
- **Judith Guest photos** — ✅ All 6 candidate photos extracted (original 3 from msg `1a082daaf4739c9d` + 3 more from msg `1a087fd2c8446918`). **Hero-photo choice is still an open decision for John** — Judy asked whether the current hero (`Unknown.jpeg`, the "sweatshirt" photo) should be replaced by the Penguin 50th-anniversary book cover (`9780143139461.jpeg`) or whether the movie-poster-frame photos should be used at all. Left as-is pending that decision; do not change without it.
- **Laurel Baer bio** — needed for about.html; her byline link is dead until it exists.

## Blockers

- **Judith Guest hero photo** — all files now extracted (see Pending Deliveries); the open item is which photo to use as hero, not file availability. Judy raised this Sept 9 and asked John to decide; not decided yet.
...
| chef | NYB Hamantaschen / Chef | Sigalit Zetouni | Placeholder (blast text only) | 1 photo on disk (`IMG_7976.JPEG`), used for nav thumb only | Sig sent 1-paragraph blast Sept 8 (msg `1a08369f62e68b76`). Full article not received. Slug may change once real title arrives. |
