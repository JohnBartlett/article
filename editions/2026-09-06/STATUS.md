...
- **Gotcha found while removing it:** `edition_checks.py`'s about.html-popup updater scans every folder under `editions/`, not just nav-linked ones, so it silently re-added the Scott Holleran/Edgelord popup entry the moment `/edition-checks` ran again. Had to remove it a second time. Any future `/edition-checks` run before this is properly resolved (published, deleted, or the script fixed to check nav-linkage) will likely re-add it — check about.html's Scott Holleran popup after running that script.
