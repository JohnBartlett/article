...
| jill-lowe-hands | A Helping Hand | Jill Lowe | ✅ Full text built (Aug 8) | ✅ 25/25 placed | "Facts and Froth" column on the anatomy, function, and expression of hands — extracted from a 15-page PDF (`65) Give a Helping Hand.pdf`) sent Aug 2, text via PyPDF2. 25 photos (21 stock + 4 personal) arrived separately Aug 3 with **no placement map** — placed by direct visual review of every photo against the article's sections (explicit go-ahead from John), not guessed from filenames. Initial pass misplaced `shutterstock_2430522269.jpeg` (the German-style three-finger count) as generic filler in Hand Signals instead of recognizing it belonged in the Shibboleth section next to the non-German photo — caught by John and corrected; both photos now sit side by side as the PDF intended. (A stand-in photo John took himself to cover the presumed gap was removed once the mistake was found.) Jill's later Aug 8 email ("send me a draft to review") matches the same build-then-review workflow used on her July "Kintsugi" piece — **flag the built draft to her for review before publishing.** |
...
- ✅ Sent Jill Lowe the preview link for "A Helping Hand" (Aug 8, msg `19fe1bfda37b7d2d`). She replied same day (`19fe238d0dacb462`) requesting her exact original photo order be restored — corrected in `jill-lowe-hands-preview/` only (not the real article yet) by rebuilding placement against all 15 pages of her original PDF; documented in `audit.html`. Awaiting her re-review of the corrected preview. Once she confirms, apply the same photo-order fix to the real `jill-lowe-hands/index.html` and delete `jill-lowe-hands-preview/` before staging.
